...
只依赖 numpy，可在本机 pytest 测；read_pcd_xyz 用 open3d，仅构建机运行。
...
输出 factory_map.yaml + 同目录同名 factory_map.pgm。
"""
import argparse
import os
...
def filter_height_band(points, z_min, z_max):
    """保留 z 落在闭区间 [z_min, z_max] 的点。points: (N,3) -> (M,3)。"""
    z = points[:, 2]
...
def read_pcd_xyz(path):  # pragma: no cover - 仅构建机(需 open3d)
...
def main(argv=None):  # pragma: no cover - CLI，仅构建机
    ...
    out_yaml = os.path.abspath(os.path.expanduser(args.out))
