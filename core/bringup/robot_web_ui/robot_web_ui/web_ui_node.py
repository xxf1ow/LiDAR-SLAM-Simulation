...
from .http_server import ActionUnavailable, create_server
...
class WebUiNode(Node):
    ...
    def takeover_manual(self) -> None:
        self._call_mode_service(self._takeover_client, "manual takeover")

    def resume_automatic(self) -> None:
        self._call_mode_service(self._resume_client, "automatic resume")

    @staticmethod
    def _call_mode_service(client, action_name: str) -> None:
        ...
        if not finished.wait(timeout=1.0):
            raise ActionUnavailable(f"{action_name} service timed out")
        if "error" in result:
