...
- **Python runs through `uv`** (binary at `~/.local/bin/uv`, on PATH), using the repo-root `.venv` it manages. Run scripts with `uv run python ...` and install deps with `uv pip ...` — don't invoke the system `python3` directly for project scripts. (Applies to the ament_python packages and ad-hoc scripts; nodes started via `ros2 launch` are unaffected.)
- **But `colcon build` must use the system `/usr/bin/python3`, NOT the `.venv` one.** The `.venv` is near-empty and `.venv/bin` sits early on `PATH`; if it leaks into a build, CMake caches `Python3_EXECUTABLE=.venv/bin/python3` and rosidl/ament fail with `No module named 'em'` / `'catkin_pkg'`. Build with `.venv` excluded from `PATH` (`export PATH=/usr/local/bin:/usr/bin:/bin:/opt/ros/humble/bin`); once it has happened, wipe `core/{build,install,log}` so the cached `Python3_EXECUTABLE` is cleared before rebuilding.
